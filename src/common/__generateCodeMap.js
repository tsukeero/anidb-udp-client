...
#CUSTOM CODES
UNKNOWN_API_ERROR                        = 670
CONNECTION_TIMEOUT                       = 671
NOT_CONNECTED                            = 672
FAILED_TO_INITIALIZE                     = 672
`
const regex = /^([A-Z_]+)\s+=\s+([0-9]+)$/gm
let m
const CODE_TO_LABEL_MAP = {}
const LABEL_TO_CODE_MAP = {}

while ((m = regex.exec(MAP_TEXT)) !== null) {
  ...
  const label = m[1]
  LABEL_TO_CODE_MAP[label] = code
  CODE_TO_LABEL_MAP[code] = label // The result can be accessed through the `m`-variable.
}

console.log(LABEL_TO_CODE_MAP)
// export default CODE_TO_LABEL_MAP; // no custom errors here!

// export const RESPONSE_CODE = { ...LABEL_TO_CODE_MAP};
